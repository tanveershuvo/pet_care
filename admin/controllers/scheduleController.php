...
if (isset($_POST['schedule_edit'])) {
    ...
    $delSql = "DELETE FROM vet_available_days WHERE `vet_id`='$id'";
    $conn->query($delSql);

    $delSql1 = "DELETE FROM vet_available_slot WHERE `vet_id`='$id'";
    $conn->query($delSql1);

    $delSql2 = "DELETE FROM vet_available_service WHERE `vet_id`='$id'";
    $conn->query($delSql2);
    ...
    $conn->query($sqlCompl);
    if ($conn->query($sql)) {
        $_SESSION['msg'] = "Schedule Edited successfully";
        $_SESSION['type'] = "success";
        header('Location:../vet-schedule');
    } else {
        $_SESSION['msg'] = "Something went Wrong!!Try again Later!";
